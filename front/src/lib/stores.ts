import { writable } from "svelte/store";
import type { TTurtle } from "./turtle.type";
import type { TBlock } from "./block.type";
import { now } from "./helper";
import type { TSlot } from "./slot.type";

export const keyboardControlEnabled = writable<boolean>(true);
export const turtleStore = writable<TTurtle | null>(null);

type TCooldown = {
  lastTriggerMs: number;
  cooldownMs: number;
};
const createCoolDownStore = (cooldownMs: number = 200) => {
  const { subscribe, set, update } = writable<TCooldown>({
    lastTriggerMs: now(),
    cooldownMs: cooldownMs,
  });

  const trigger = () => {
    update(({ cooldownMs }) => {
      return {
        lastTriggerMs: now(),
        cooldownMs,
      };
    });
  };

  const inactiveAt = (lastTriggerMs: number, cooldownMs: number) => {
    return lastTriggerMs + cooldownMs;
  };

  const isActive = () => {
    let active = false;
    subscribe(({ lastTriggerMs, cooldownMs }) => {
      active = now() - lastTriggerMs < cooldownMs;
    });
    return active;
  };

  const timeLeft = () => {
    let timeLeftMs = 0;
    subscribe(
      ({ lastTriggerMs, cooldownMs }) =>
        (timeLeftMs = inactiveAt(lastTriggerMs, cooldownMs) - now())
    );
    return Math.abs(timeLeftMs);
  };

  return {
    subscribe,
    set,
    update,
    trigger,
    isActive,
    timeLeft,
  };
};

export const cooldownStore = createCoolDownStore(500);

const createWorldStore = () => {
  const { subscribe, set, update } = writable<TBlock[]>([]);

  const comparePos = (a: TBlock, b: TBlock) =>
    a.x === b.x && a.y === b.y && a.z === b.z;

  const add = (newBlocks?: TBlock[]) => {
    if (!newBlocks) return;
    update((blocks) => {
      for (const newBlock of newBlocks) {
        blocks = blocks.filter((block) => !comparePos(block, newBlock));
        blocks.push(newBlock);
      }

      return blocks;
    });
  };

  let remove = (blocksToRemove?: TBlock[]) => {
    if (!blocksToRemove) return;
    update((blocks) => {
      for (const blockToRemove of blocksToRemove) {
        blocks = blocks.filter((block) => !comparePos(blockToRemove, block));
      }

      return blocks;
    });
  };

  const get = () => {
    let world: TBlock[] = [];
    subscribe((blocks) => (world = blocks));
    return world;
  };

  return {
    subscribe,
    set,
    update,
    add,
    remove,
    get,
  };
};

export const worldStore = createWorldStore();

const createInventoryStore = () => {
  const { subscribe, set, update } = writable<TSlot[]>([]);

  return {
    subscribe,
    set,
    update,
  };
};

export const inventoryStore = createInventoryStore();
